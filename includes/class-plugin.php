...
class Plugin {
	...
	private function load_textdomain() {
		load_plugin_textdomain(
			'ninja-gallery',
			false,
			dirname( PNPNG_BASENAME ) . '/languages'
		);
	}
	...
	private function boot_subsystems() {
		...
		// Front-end shortcode.
		add_shortcode( 'ninja_gallery', array( $this, 'render_shortcode' ) );
	}

	/**
	 * Render [ninja_gallery id="X" layout="grid"] shortcode.
	 *
	...
	public function render_shortcode( $atts, $content = '' ) {
		...
		if ( ! $gallery_id ) {
			return '<p>' . esc_html__( 'Please provide a gallery ID.', 'ninja-gallery' ) . '</p>';
		}
